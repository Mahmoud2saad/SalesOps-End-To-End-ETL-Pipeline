# NOTE: an earlier, fully-commented draft of this script (~340 lines) was removed
# during cleanup. See dev_code/1) Scripts/legacy/ for older bronze-layer iterations
# if you need history; this file now contains only the working version.
...
# =====================================================================
# MAIN SILVER INCREMENTAL LOAD LOGIC (SIMPLIFIED - LIKE WORKING CODE)
# =====================================================================

def run_silver_incremental_load(target_year):
    # Ensure year format is correct
    year_str = str(target_year).replace("batch_", "").replace("year_", "")
    print(f"🚀 Starting Silver Layer Incremental Load for BATCH/YEAR {year_str} ...")
    print("="*80)

    # Initialize Spark
    spark = SparkSession.builder \
        .appName(f"Silver_Inc_Load_{year_str}") \
    ...
    # Path for silver incremental data (SAME PATTERN AS BRONZE)
    silver_inc_dir = f'/home/jovyan/data/silver/increments/batch_{year_str}'
    temp_dir_base = '/home/jovyan/work/temp_silver_export_inc'
    
    # Tables to process (simplified like working code)
    tables_config = {
        'orders': {
            'inc_col': 'o_orderdate',
            'is_date': True,
            'table_name_full': 'silver.orders'
        },
        'lineitem': {
            'inc_col': 'l_shipdate',
            'is_date': True,
            'table_name_full': 'silver.lineitem'
        }
    }

    # PostgreSQL connection (SAME AS WORKING CODE)
    ...
    cur = pg_conn.cursor()
    execution_id = f'silver_inc_load_batch_{year_str}'

    for table, config in tables_config.items():
        # IMPORTANT: Use EXACT same file pattern as working code
        file_pattern = f"{silver_inc_dir}/**/{table}.parquet"
        inc_col = config['inc_col']
        is_date = config['is_date']
        incremental_type = 'date' if is_date else 'numeric'
        table_name_full = config['table_name_full']
        temp_dir = f"{temp_dir_base}_{table}"
        
        # Find the parquet file (SAME AS WORKING CODE)
        matched_files = glob.glob(file_pattern, recursive=True)
        
        if matched_files:
            actual_file_path = matched_files[0]  # Take first match
            print(f"\n{'='*50}")
            print(f"🔍 Processing {table_name_full} from {actual_file_path}...")
            
            # Get watermark (SAME AS WORKING CODE)
            ...
            audit_id = audit_logger.log_start("Silver_Incremental_Load", execution_id, f"silver_inc_{table}", table_name_full)
            
            try:
                # Read ONLY the specific parquet file (SAME AS WORKING CODE)
                df_source = spark.read.parquet(actual_file_path)
                
                # MINIMAL transformations (like working code)
                # Only add surrogate key if needed
                if table == 'lineitem':
                    if 'l_id' in df_source.columns:
                        df_source = df_source.drop('l_id')
                    df_source = df_source.withColumn("l_id", F.monotonically_increasing_id())
                
                # Filter for new data (SAME AS WORKING CODE)
                df_new = df_source.filter(F.col(inc_col) > current_wm)
                new_rows_count = df_new.count()
                
                if new_rows_count == 0:
                    print(f"   🛑 No new data found > {current_wm}. Skipping.")
                    audit_logger.log_complete(audit_id, status='SUCCESS', rows_processed=0)
                    continue
                
                print(f"   ⚡ Found {new_rows_count} NEW rows!")
                
                # Export to CSV (SAME AS WORKING CODE)
                print(f"   💾 Exporting to CSV...")
                df_new.write.csv(temp_dir, header=True, mode="overwrite", quote='"', escape='"')
                
                # Load using COPY (SAME AS WORKING CODE)
                ...
                # Calculate new watermark (SAME AS WORKING CODE)
                ...
                print(f"   🔖 Updating Watermark to: {new_max_val}")
                
                formatted_new_max = str(new_max_val)
                
                # Update watermark (SAME AS WORKING CODE)
                upsert_query = text("""
                ...
                print(f"   ✅ Silver load for {table_name_full} completed!")
            ...
            print(f"\n⚠️ No file found for {table} at: {file_pattern}")

    cur.close()
    pg_conn.close()
    spark.stop()
    
    print(f"\n🎉 SILVER INCREMENTAL LOAD FOR BATCH {year_str} COMPLETE!")
...
if __name__ == "__main__":
    ...
    run_silver_incremental_load(target_input)